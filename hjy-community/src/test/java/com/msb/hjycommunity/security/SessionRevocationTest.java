...
class SessionRevocationTest {
    ...
    @Test void adminSessionReloadsPermissionsAndRejectsChangedCredentials() {
        ...
        MockHttpServletRequest druidRequest = new MockHttpServletRequest("GET", "/druid/index.html");
        druidRequest.setCookies(new Cookie("Admin-Token", token));
        assertEquals(2L, service.getLoginUser(druidRequest).getUser().getUserId());
        MockHttpServletRequest unrelatedCookieRequest = new MockHttpServletRequest("GET", "/system/user/list");
        unrelatedCookieRequest.setCookies(new Cookie("Admin-Token", token));
        assertNull(service.getLoginUser(unrelatedCookieRequest));
