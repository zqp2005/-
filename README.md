...
完整演示步骤、反例与测试命令见 [毕设核心业务与测试指南](doc/毕设核心业务与测试指南.md)。
...
- 小程序通过 `POST /app/activation/preview` 以激活码查询待开通档案，只回显姓名和脱敏手机号，不下发身份证号或密码。提交 `POST /app/register` 时只传激活码和新密码，后端重新校验并原子消耗激活码，再为原档案设置密码。普通新手机号注册仍使用姓名、手机号和密码。
