...
- 恢复受控的 Druid 数据监控：入口和服务端路径均要求当前平台超级管理员登录，再使用独立 Druid 账号密码二次认证；恢复 AI 确认后创建报修和当前用户登录位置查询。AI 会话改用稳定用户 ID，清空页面与删除 Redis 记忆分离；报修草稿按用户会话在 Redis 保留 10 分钟，确认时原子取出，防止跳过预览或重复提交。
