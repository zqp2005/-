...
### 后台居民档案开通小程序（2026-09-25）

- 后台先建档的手机号不能公开自助认领。物业线下核验本人和手机号后，在业主信息中点“开通小程序”，获取 15 分钟有效的一次性激活码并线下交给居民。
- 居民在小程序注册页填原手机号、档案姓名、新密码和激活码；系统原地更新 `hjy_owner` 的密码与启用状态，不新建重复档案，已开通账号不能重置。普通新手机号注册仍无需激活码。
- 激活码摘要仅暂存 Redis，使用后原子删除；没有短信核验，物业必须实际核验身份并妥善交付激活码。未做数据库迁移，不自动处理历史重复手机号。
- 验证：JDK 8 下 17 项后端定向测试通过，前端 Vite 生产构建和 5 项轻量测试通过，小程序注册脚本语法检查通过；尚未在浏览器/微信开发者工具完成真实账号端到端验收。测试未启动服务，未占用端口。

本地测试约定：测试时临时启动的服务由启动者在测试后关闭，并确认端口释放；不要停止其他人原本运行的服务。

### 本批核心修复（2026-09-24）

- 已从当前数据库删除“数据监控”菜单 111，左侧导航不再显示；Druid 服务配置保持不变。
- 恢复受控的 Druid 数据监控：服务端路径仍要求当前平台超级管理员登录，再使用独立 Druid 账号密码二次认证；恢复 AI 确认后创建报修和当前用户登录位置查询。AI 会话改用稳定用户 ID，清空页面与删除 Redis 记忆分离；报修草稿按用户会话在 Redis 保留 10 分钟，确认时原子取出，防止跳过预览或重复提交。
- 主后端日志目录默认保持 `/home/hejiayun/logs`，Windows 本地运行可通过 `LOG_PATH` 指定可写目录。
...
- 补齐物业工作台首页：聚合待审核人房关联、待派单报修、处理中报修和待受理投诉，卡片可直达对应业务页面；数据按当前账号有列表权限的接口读取，具体动作仍由后端权限和状态机控制。前端 Vite 生产构建通过，浏览器端尚待物业账号验收。
...
- **注册限制**：新手机号可注册；已注册手机号拒绝；存量业主（密码为空）须经物业线下核验并凭 15 分钟一次性激活码开通，不能仅凭自填手机号和姓名认领，也不能通过普通档案编辑设置密码绕过限制。
...
部署注意（历史第 1A 批）：当时无数据库迁移、未改前端或小程序；存量居民当时尚不能激活。2026-09-25 已补物业发码和小程序激活流程，仍需配套发布后端、前端与小程序后才生效；尚未执行云端部署或完整浏览器端到端验收。
