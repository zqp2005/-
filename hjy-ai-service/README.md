...
- AI 会话使用稳定用户 ID，重新登录后继续原会话；前端“清空显示”保留 Redis 上下文，“彻底忘记”才删除服务端记忆和待确认报修。创建报修先把草稿在 Redis 保留 10 分钟并展示核对信息，只有同一用户、同一会话明确回复“确认提交”才透传当前用户令牌执行；登录位置只能查询当前 JWT 对应账号。
...
- 创建报修已接入二次确认；取消工单、投诉和访客写入仍通过业务页面提交。登录定位强制使用当前账号，天气仍可查询。
- 费用工具未接入账单，明确返回暂不可查询，不给出虚构欠费、费率、联系电话或支付入口。报修/投诉列表按后端字段服务端筛选后分页。
- 与主后端配套部署：`/aiLogin` 已关闭，旧会话需重新登录；AI 入口调用主后端 `/getInfo` 验证账号实时状态。
- 验证：`mvn test` 20 项通过，覆盖 SSE、JWT、工具授权、报修二次确认、登录用户名覆盖、HTTP 方法及真实性约束；单元测试不调用外部模型、不写业务数据库。
